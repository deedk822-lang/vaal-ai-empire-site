...
class VaalAIEmpireOrchestrator:
    ...
    def __init__(
        self, llm_config_path: str = "OAI_CONFIG_LIST", cohere_api_key: str = None
    ):
    ...
    async def initialize(self):
        ...
        self.financial_sentinel = FinancialSentinelAgent(
            llm_config=self.llm_config, cohere_api_key=self.cohere_api_key
        )
        ...
        print(
            "[Orchestrator] ✅ Vaal AI Empire ready! Financial Sentinel + Guardian Engine online."
        )

    async def auto_pattern(
        self, task: str, max_rounds: int = 20, require_human_approval: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        response = run_group_chat(pattern=pattern, messages=task, max_rounds=max_rounds)
    ...
    async def sequential_workflow(
        self, task: str, workflow_steps: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for step_name in workflow_steps:
            ...
            # Run agent
            response = agent.run(recipient=agent, message=current_message, max_turns=5)
    ...
    async def tax_recovery_workflow(
        ...
        financial_response = self.financial_sentinel.agent.run(
            recipient=self.financial_sentinel.agent, message=task, max_turns=5
        )
        ...
        if "current_eaf" in company_data:
            ...
            crisis_response = self.guardian_engine.agent.run(
                recipient=self.guardian_engine.agent, message=crisis_task, max_turns=3
            )
        else:
            crisis_response = (
                "No current crisis data provided. Skipping risk assessment."
            )

        # Step 3: Combine results
        return {
            "tax_recovery": (
                financial_response.summary
                if hasattr(financial_response, "summary")
                else str(financial_response)
            ),
            "crisis_assessment": (
                crisis_response.summary
                if hasattr(crisis_response, "summary")
                else str(crisis_response)
            ),
    ...
    def get_agent_status(self) -> Dict[str, str]:
        ...
        return {
            "financial_sentinel": (
                "✅ Online"
                if self.financial_sentinel and self.financial_sentinel.initialized
                else "❌ Offline"
            ),
            "guardian_engine": (
                "✅ Online"
                if self.guardian_engine and self.guardian_engine.initialized
                else "❌ Offline"
            ),
            "human_proxy": "✅ Ready" if self.human_proxy else "❌ Not configured",
            "orchestrator": (
                "✅ Initialized" if self.initialized else "❌ Not initialized"
            ),
        }
