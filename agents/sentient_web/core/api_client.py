...
class GLM5Client:
    ...
    def __init__(
        ...
        self.circuit_breaker = CircuitBreaker(
            failure_threshold=3, recovery_timeout=60.0, name="glm5_api"
        )
    ...
    def _get_from_cache(self, cache_key: str) -> Optional[APIResponse]:
        ...
        if cached:
            ...
            if age < self.cache_ttl:
                ...
                return APIResponse(
                    success=True, data=cached["data"], cached=True, latency_ms=0
                )
            else:
        ...
        for attempt in range(self.retry_policy.max_attempts):
            try:
                response = await self.circuit_breaker.call(
                    self._make_request, headers, payload
                )
    ...
    async def _make_request(
        ...
        async with session.post(
            self.GLM5_API_URL, headers=headers, json=payload
        ) as response:
    ...
    async def _fallback_generate(
        self, prompt: str, original_error: Exception
    ) -> APIResponse:
        """
...
""".strip()
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        success_rate = (
            self.metrics["requests_success"] / total * 100 if total > 0 else 0
        )
        avg_latency = (
