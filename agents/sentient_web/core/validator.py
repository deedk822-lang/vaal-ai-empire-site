...
class SecurityScanner:
    ...
    DANGEROUS_PATTERNS = {
        "javascript": {
            ...
            "insert_adjacent_html": re.compile(
                r"\.insertAdjacentHTML\s*\(", re.IGNORECASE
            ),
            "set_timeout_string": re.compile(r'setTimeout\s*\(\s*["\']', re.IGNORECASE),
            "set_interval_string": re.compile(
                r'setInterval\s*\(\s*["\']', re.IGNORECASE
            ),
        ...
        "python": {
            ...
            "subprocess_shell": re.compile(
                r"subprocess\.\w+\s*\([^)]*shell\s*=\s*True", re.IGNORECASE
            ),
    ...
    def generate_report(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "high": high,
                "medium": sum(
                    1 for r in results for i in r.issues if i.severity == "medium"
                ),
