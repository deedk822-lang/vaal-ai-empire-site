...
class BenchmarkRunner(ABC):
    ...
    def _check_threshold(
        self, value: float, threshold: Optional[float], lower_is_better: bool = True
    ) -> str:
    ...
    async def run(self, url_or_path: str) -> BenchmarkResult:
        ...
        if not await self._check_lighthouse():
            return self._fallback_result(
                url_or_path, timestamp, "Lighthouse not installed"
            )
...
class AXEAccessibilityRunner(BenchmarkRunner):
    ...
    async def run(self, url_or_path: str) -> BenchmarkResult:
        ...
        if is_file:
            ...
            if not server_url:
                return self._fallback_result(
                    url_or_path, timestamp, "Could not serve file"
                )
        else:
        ...
        try:
            ...
            if not await self._check_axe():
                return self._fallback_result(
                    url_or_path, timestamp, "axe not installed"
                )
            ...
            if process.returncode not in [0, 1]:  # 1 means violations found
                return self._fallback_result(
                    url_or_path, timestamp, f"axe failed: {stderr.decode()[:200]}"
                )
    ...
    async def _serve_file(
        self, file_path: str
    ) -> Tuple[Optional[str], Optional[HTTPServer]]:
        """
    ...
    def _aggregate_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "all_metrics": [
                {
                    "tool": next(
                        (r.tool for r in self.results if m in r.metrics), "unknown"
                    ),
                    "name": m.name,
                    "value": m.value,
                    "unit": m.unit,
