...
async def test_tracer():
    ...
    print(
        f"✅ Trace created with {len(otlp['resourceSpans'][0]['scopeSpans'][0]['spans'])} spans"
    )
...
async def test_swarm_orchestrator():
    ...
    config = SwarmConfig(
        project_name="test-project", output_dir="output/test", run_code_review=False
    )
    ...
    try:
        result = await orchestrator.run(
            context={"company_name": "Test Company", "description": "AI testing"}
        )

        print(f"\n✅ Swarm completed in {result.duration_seconds:.2f}s")
        print(
            f"   Agents successful: {sum(1 for r in result.agent_results if r.get('status') == 'success')}/{len(result.agent_results)}"
        )
