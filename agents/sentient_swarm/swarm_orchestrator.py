...
from .observability import EnterpriseMetrics, DistributedTracer, StructuredLogger
from .agents import (
    SentientUIAgent,
    MXAgent,
    EmpathyAgent,
...
class SwarmOrchestrator:
    ...
    async def run(self, context: Optional[Dict[str, Any]] = None) -> SwarmResult:
        ...
        async def run_agent(name: str, agent):
            async with semaphore:
                ...
                try:
                    ...
                    self.metrics.counter("agent_executions", 1, {"agent": name, "status": "success"})
                    print(f"✅ {name}: Success")
                    return result
                except Exception as e:
                    self.tracer.end_span(span_id, status="error", error=str(e))
                    self.logger.exception(f"Agent {name} failed", e)
                    self.metrics.counter("agent_executions", 1, {"agent": name, "status": "error"})
        ...
        print(f"Agents: {len([r for r in agent_results if r.get('status') == 'success'])}/{len(agent_results)}")
        print(f"Files generated: {sum(len(r.get('files', [])) for r in agent_results)}")
        print(f"LLM providers used: {', '.join(llm_metrics.get('available_providers', []))}")
