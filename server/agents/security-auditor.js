...
const audits = {
  ...
  'Rate limiting configuration': () => {
    ...
    if (!serverCode.includes('express-rate-limit')) {
      return {
        level: 'HIGH',
        message:
          'Rate limiting is not implemented, leaving the server vulnerable to brute-force and DoS attacks.',
  ...
  'XSS/NoSQL injection protection': () => {
    ...
    if (!xssFound || !mongoSanitizeFound) {
      let message = '';
      if (!xssFound) {
        message += 'XSS cleaning middleware is missing. ';
      }
      if (!mongoSanitizeFound) {
        message += 'NoSQL injection sanitization is missing.';
      }
      return {
        level: 'HIGH',
        message,
  ...
  'Payload size limits': () => {
    ...
    if (!serverCode.includes("limit: '10kb'")) {
      return {
        level: 'MEDIUM',
        message:
          'Payload size limits are not configured. The server may be vulnerable to DoS attacks via large request bodies.',
